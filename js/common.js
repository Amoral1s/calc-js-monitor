...
calcStepItem.forEach((elem,i) => {
  ...
  //BUTTONS
    ...
    let secondSectionFunc = function() {
      myFadeIn(calcPage[i]);
    ...
    let buttonPrev4 = calcPage[i].querySelector('.calc-page__prev'),
        buttonNext4 = calcPage[i].querySelector('.calc-page__next');
//BUTTONS
    buttonNext4.disabled = true; 
    ...
    let calcPageIClick = function(e) {
      ...
      if(target == buttonNext4 && buttonNext4.disabled == false) { //NEXT BUTTON
        if (pageCount4 >= calcPageStepsStart.length - 2) {
          buttonNext.style.display = 'none';
        }
        calcPageStepsStart.forEach((elem) => {
          elem.style.display = 'none'
      ...
      if(target == buttonPrev4) {//PREV BUTTON
        calcPageStepsStart.forEach((elem) => { 
          elem.style.display = 'none'
        });
        buttonNext4.disabled = false; 
        ...
        if (pageCount == calcPageStepsStart.length - 1 && calcPageStepsStart.length > 2) {
          buttonNext4.style.display = 'block';
          pageCount--;
          myFadeIn(calcPageStepsStart[pageCount]);
        } else if (pageCount == 0) {
          window.location.reload()
...
const gifsWrap = document.querySelector('.gifs-wrap'),
      gifs = document.querySelectorAll('.gifs-wrap .gifs div'),
      gifsInner = document.querySelectorAll('.calc-diag-arrows .gifs div'),
